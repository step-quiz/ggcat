  ...
  function _injectApplet(wrapper, doneCb) {
    ...
    var params = {
      ...
      appletOnLoad:        function(api) {
        ...
        // Perspectiva: sempre vista gràfica pura (amaga el panel d'àlgebra
        // lateral que el geometry app mostra per defecte).
        // 'G' = Graphics view only; ho fem PRIMER perquè les comandes
        // subsegüents que creen objectes no reobrin el panel.
        try { api.setPerspective('G'); } catch(e) {}

        // Reconcilia la mida: entre la creació de l'applet i ara el
        // contenidor pot tenir una amplada lleugerament diferent.
        var finalW = container.offsetWidth;
        var finalH = container.offsetHeight;
        if (finalW > 50 && finalH > 50) {
          try { api.setSize(finalW, finalH); } catch(e) {}
        }
